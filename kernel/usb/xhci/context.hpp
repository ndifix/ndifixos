...
union EndpointContext {
  ...
  uint64_t TransferRing() const { return bits.tr_dequeue_pointer << 4; }

  void SetTransferRing(uint64_t value) { bits.tr_dequeue_pointer = value >> 4; }
} __attribute__((packed));
...
struct InputContext {
  ...
  // <returns> Pointer to the slot context enabled.</returns>
  ...
  // Pointer to the endpoint context enabled.
